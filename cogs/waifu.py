...
from ui_elements import Hmtai, WaifuIm
...
class WaifuCog(commands.GroupCog, name="waifu"):
    ...
    async def hm_tai_command(self, i: Interaction, num: int, tag_type: str):
        ...
        view = Hmtai.View(self.tags[tag_type], num)
    ...
    async def waifu(self, i: Interaction, many: int = 0, sese: int = 0, tags: int = 0):
        ...
        if tags == 1:
            view = WaifuIm.View(await get_waifu_im_tags(sese, i.client.session), i.user)
            await i.followup.send(view=view, ephemeral=True)
            view.message = await i.original_response()
            await view.wait()
            if not view.tags:
        ...
        await i.edit_original_response(
            embed=default_embed().set_author(name="正在尋找圖片...", icon_url="https://i.imgur.com/V76M9Wa.gif"), view=None
        )
        async with WaifuAioClient() as wf:
            ...
            for index, image in enumerate(images):
                ...
                embeds.append(embed)
            await Paginator(i, embeds).start(edit=True)
